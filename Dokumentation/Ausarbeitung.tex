...
\\
Beispiele für Fließkommaintensive Berechnungen sind Audioverarbeitung, 3D-Visuali-sierungen oder Physik-Simulationen. %% Der Bindestrich hier hindert latex daran, doof zu sein
\\ \\
...
In der von uns enwickelten Lösung verwenden wir die \emph{Vector Floating Point}-Befehle, diese erlaubt es uns mit Fließkommazahlen zu rechnen.
\subsection*{Vorteile}
\begin{itemize}
...
zu profitieren. Das bedeutet, dass pro Instruktion nicht mit einer, sondern vier Zahlen gleichzeitig gerechnet werden kann. Unsere Tests haben gezeigt, dass die Berechnung dadurch mindestens um den Faktor 6 beschleunigt wird.
...
Die einmalige Annäherung ist dadurch nicht sehr genau, durch mehrmalige Iteration des Newton-Raphson-Verfahrens mittels \emph{VRECPS} erhält man jedoch größere Genauigkeit.
\end{itemize}
...
Durch Vorberechnung des konstanten Faktors der Formel ($4 \cdot \pi \cdot \varepsilon_{0}$) kann dieser in allen späteren Berechnungen wiederverwendet werden, was einigen Aufwand erspart. Diese Optimierung wurde nur in der VFP-Variante umgesetzt, da hier auch eine Schleifenstruktur implementiert wurde. Bei der NEON-Alternative ist also zusätzliches Optimierungspotential gegeben.\\
Die effiziente (minimale) Verwendung der verfügbaren Register bietet weiterhin Optimierungsmöglichkeiten. Hier wurde versucht den idealen Kompromiss zwischen Effizienz und Lesbarkeit zu finden.\\
