...
Dann werden drei verschiedene Implementierungsvarianten ausgeführt (Assembler mit VFP, Assembler mit NEON und Vektorisierung und die C-Referenzimplementierung), deren Ergebnisse ausgegeben werden und die dabei benötigte Zeit (in Mikrosekunden) gemessen wird.
\\\\
...
Hierbei ist $x$ mit der Mächtigkeit $\vert x \vert = n$ die Menge der zu berechnenden Datensätze bestehend aus den 2-Tupeln $(r_{1},r_{2})$, wobei $r_{1}$ den Radius der inneren Kugel, $r_{2}$ den der äußeren Kugel bezeichnet.
\subsubsection{Assemblerroutine - VFP}
Die calc-Methode erwartet 
%%loop
%%capacity
%%TODO
